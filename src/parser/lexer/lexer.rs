    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let firstToken = match c {
                ...
                c => {
                    if c.is_alphabetic() {
                        return Some(self.word_startded());
                    }
...
impl<'a> Lexer<'a> {
    ...
    pub fn single(&mut self, token: Token<'a>) -> Result<Token<'a>, LexError> {
    ...
    fn may_by_longer(&mut self, first: Token<'a>) -> Result<Token<'a>, LexError> {
    ...
    fn word_startded(&mut self) -> Result<Token<'a>, LexError> {
